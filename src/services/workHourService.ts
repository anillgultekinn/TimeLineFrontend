...
class WorkHourService extends BaseService<
    ...
    getByFilter(request: WorkHourFilterRequest, pageIndex: number, pageSize: number): Promise<AxiosResponse<Paginate<GetListWorkHourResponse>, any>> {
        return axiosInstance.post<Paginate<GetListWorkHourResponse>>(this.apiUrl + "/GetListByFiltered?PageIndex=" + pageIndex + "&PageSize=" + pageSize, request);
    }
}
